...
WORKDIR /templates/service/host/ECommerce.Customers.Host
RUN dotnet restore
RUN dotnet ef migrations script -i -o migrations-Customers.sql
...
COPY --from=build /templates/service/host/ECommerce.Customers.Host/migrations-Customers.sql migrations-Customers.sql
