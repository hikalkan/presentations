...
WORKDIR /templates/service/host/ECommerce.Ordering.Host
RUN dotnet restore
RUN dotnet ef migrations script -i -o migrations-Ordering.sql
...
COPY --from=build /templates/service/host/ECommerce.Ordering.Host/migrations-Ordering.sql migrations-Ordering.sql
